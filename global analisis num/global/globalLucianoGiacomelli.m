function globalLucianoGiacomelli
...
escalon = zeros(N);
    ...
    for i = 1:N
        ...
        switch (i)
            case 1
                derivada(1) = (1 / (2 * Dt)) * (-3 * escalon(1) + 4 * escalon(2) - escalon(3));
            case N
                derivada(N) = (1 / (2 * Dt)) * (3 * escalon(N) - 4 * escalon(N - 1) + escalon(N - 2));
            otherwise
                derivada(i) = (1 / (2 * Dt)) * (-escalon(i - 1) + escalon(i + 1));
