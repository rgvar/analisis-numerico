...
% Encontrar g_max y t_gmax en el rango t_n desde 0 hasta t(N/2)
N = length(tn)
tn_limit = tn(ceil(N/2));
...
threshold = 0.10 * G_tdf_MAX; % Umbral del 10% de G_tdf_MAX
...
% Paso 5: Aproximación de Mínimos Cuadrados de g_n usando 3 frecuencias
...
Phi = ones(N, 2*3 + 1); % Matriz Phi con la base de funciones
for k = 1:3
    Phi(:, 2*k) = cos(k * Dw * tn);
...
% Encontrar Pa_max y t_Pa_max en el rango t_n desde 0 hasta t(N/2)
...
% Encontrar yc_max y t_yc_max en el rango t_n desde 0 hasta t(N/2)
